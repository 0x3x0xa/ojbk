...
function lottery_qishu($type) {
	...
	if($qs) {
		if($type==15){
			...
			return date("Ymd",$lottery_timeffc).BuLingss($qs['qishu']);
		}else{
...
function lottery_qishu15($type) {
	$type = intval($type);
    global $lottery_timeffc;
	
	global $mysqli;
	$sql	=	"select qishu from c_opentime_".$type." where kaipan<='".date("H:i:s",$lottery_timeffc)."' and kaijiang>='".date("H:i:s",$lottery_timeffc)."' limit 1";
	$query	=	$mysqli->query($sql);
	$qs		=	$query->fetch_array();
	if($qs) {
...
function lottery_qishu_13($type) {
    ...
    $curl->set_referrer("http://old.1680180.com/lottery/10041.html");
    ...
    $html_data = $curl->fetch_url("http://old.1680180.com/Open/CurrentOpen?code=10041");
