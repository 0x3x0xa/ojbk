...
function message($value,$url=""){ //默认返回上一页
	echo '<font color="red" style="font-size:12px;">'.$value.'</font>';exit;
}
$isfs=0;
...
//获取开奖号码
$sql		= "select * from c_auto_17 where qishu=".$qi." order by id desc limit 1";
...
$hm[]		= $rs['ball_6'];
$hm[]		= $rs['ball_7'];
if(!$rs['ball_1'] || !$rs['ball_7']){
	message($qi.'尚未开奖,或者开奖号没录入');
}
...
while($rows = $query1->fetch_array()){
	...
	$n_date=date("Y-m-d");
	$msql2="update c_bet set bet_date='$n_date',daywin=win,daymoney=money where id=".$rows['id']."";
$mysqli->query($msql2);
	//填写开奖结果到注单
...
$msql="update c_auto_17 set ok=1 where qishu=".$qi."";
