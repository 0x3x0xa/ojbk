...
function timer(iTime) {
	...
	if(iTime < 0) {
		...
		if(t > 0) {
			...
			$('#hour_show').html(00+'&nbsp;时');
			$('#minute_show').html(00+'&nbsp;分');
			$('#second_show').html(00+'&nbsp;秒');
		}
		
		fp = setTimeout("timer(" + iTime + ")", 1000);
	}
}
...
function order() {
	...
	var money;
	if($('input:radio[name="ball_11"]:checked').val() == 1) {		
		money = parseFloat($("#money").html());
		if((money +m) > 5) {
	        layer.msg("四全中当期最大下注金额不能大于5元！！！");
		    return false;
		}
	}
	...
	if(typein == '4') {
		if ($("input[name='ball_sx']:checked").length < 1) {
            layer.msg('请至少选择 1 组生肖！');
            return false;
		}
		if ($("input[name='ball_ws']:checked").length < 1) {
            layer.msg('请至少选择 1 组尾数！');
            return false;
		}
	...
	if(typein == '5') {
		if ($("input[name='ball_dm[]']:checked").length < 1) {
            layer.msg('请至少选择 1 个胆码！');
            return false;
		}
		if(hm > 2) {
			if ($("input[name='ball_dm[]']:checked").length+$("input[name='ball_tm[]']:checked").length < hm) {
				layer.msg('胆码+拖码至少选择 ' + hm + ' 个号码！');
                return false;
			}
		} else {
			if ($("input[name='ball_tm[]']:checked").length < 1) {
				layer.msg('请至少选择 1 个拖码！');
                return false;
			}
		}
		var dm = [];
